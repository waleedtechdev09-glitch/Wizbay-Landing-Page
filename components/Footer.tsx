...
const Footer = () => {
  // Navigation Columns mapping directly to the mockup layout data
  ...
  return (
    <footer className="w-full bg-[#3A4041] font-sans text-white pt-16 pb-6">
      <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-10">
        {/* =========================================================
            MAIN ROW: BRAND DISCLOSURE & 4 COLUMN NAVIGATION LINKS
           ========================================================= */}
        <div className="grid grid-cols-1 gap-10 md:grid-cols-3 lg:grid-cols-6 lg:gap-8">
          {/* Column 1 & 2: Brand Tagline Statement Box */}
          <div className="md:col-span-1 lg:col-span-2 flex flex-col items-start pr-4">
            <span className="font-serif text-[26px] font-black tracking-wider uppercase select-none">
              WIZBAY
            </span>
            <p className="mt-4 max-w-[280px] text-[13px] font-normal leading-relaxed text-[#D1D5D6]">
              Discover styles that fit your identity — from women's fashion to
              men's essentials.
            </p>
          </div>

          {/* Columns 3 to 6: Loop through the defined link lists matrix structural layout */}
          {navigationLinks.map((col, index) => (
            <div key={index} className="flex flex-col items-start">
              <h4 className="text-[14px] font-semibold tracking-wide mb-4 text-white">
                {col.title}
              </h4>
        ...
        {/* =========================================================
            MIDDLE SECTION: BRANDS META (PAYMENTS, CARRIERS, SOCIALS)
           ========================================================= */}
        <div className="mt-16 flex flex-col gap-8 border-b border-[#52595A] pb-8 lg:flex-row lg:items-center lg:justify-between">
          {/* Payment & Carrier Logos Wrapper block */}
          <div className="flex flex-wrap gap-x-12 gap-y-6">
            {/* Payment Methods Indicator Container Box */}
            <div className="flex flex-col gap-2">
              <span className="text-[11px] font-medium tracking-wider text-[#A0A7A8] uppercase">
                Our Payment Methods
              </span>
              <div className="flex flex-wrap items-center gap-2">
                {[
                  ...
                  <div
                    key={idx}
                    className="flex h-7 items-center justify-center rounded bg-white/10 px-2.5 text-[10px] font-bold text-white tracking-wider select-none"
                  >
                    {pay}
                  </div>
                ))}
            ...
            {/* Carrier Shipping Methods Indicator Container Box */}
            <div className="flex flex-col gap-2">
              <span className="text-[11px] font-medium tracking-wider text-[#A0A7A8] uppercase">
                We Ship With
              </span>
              <div className="flex flex-wrap items-center gap-2">
                {["UPS", "DHL", "FedEx", "ACS", "TNT", "Aramex"].map(
                  (ship, idx) => (
                    <div
                      key={idx}
                      className="flex h-7 items-center justify-center rounded bg-white/10 px-2.5 text-[10px] font-semibold text-white tracking-wide select-none"
                    >
                      {ship}
                    </div>
                  ),
          ...
          {/* Social Media Link Icons Rounded Circles Tray element row */}
          <div className="flex items-center gap-2.5 self-start lg:self-end">
            {[
              ...
              {
                label: "TikTok",
                icon: "M12.525.02c1.31.03 2.61.37 3.77 1.01a6.76 6.76 0 01-2.02 4.3 6.7 6.7 0 01-4.73 1.94V10c2.19-.04 4.22-.84 5.82-2.26v8.43a6.83 6.83 0 01-6.83 6.83 6.83 6.83 0 01-6.83-6.83 6.83 6.83 0 016.83-6.83v3.3a3.53 3.53 0 00-3.53 3.53 3.53 3.53 0 003.53 3.53 3.53 3.53 0 003.53-3.53V0h3.94z",
              },
            ].map((soc, idx) => (
              <Link
                key={idx}
                href={`#social-${soc.label.toLowerCase()}`}
        ...
        {/* =========================================================
            BOTTOM BAR: COPYRIGHT BANNER & REGIONAL LOCALIZATION
           ========================================================= */}
        <div className="mt-6 flex flex-col gap-4 text-[12px] font-normal text-[#C4C9CA] sm:flex-row sm:items-center sm:justify-between">
          ...
          {/* Legal Compliance & Regional Selectors Shell */}
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            ...
            {/* Border Spacer line divider */}
            <span className="hidden h-4 w-[1px] bg-[#52595A] sm:inline-block" />

            {/* Selector Dropdowns matching standard localized layout properties */}
